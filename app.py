    ...
    if _show_scan and _stocks_dict:
        ...
        if not _scan_rows:
            ...
            if _purple_rows_full:
                ...
                with st.expander("🎓 شرح أعمدة **التوصية + السبب** — اضغط للتوسيع"):
                    st.markdown("""
### 🎯 التوصية (4 حالات واضحة فقط)
| التوصية | متى | ماذا تفعل |
|---|---|---|
| ✅ **ادخل** | دعم بنفسجي + (تجميع أو نابض) | اشترِ — أعلى احتمال نجاح |
| ⚠️ **راقب** | دعم/مقاومة لكن إشارات غير مكتملة | انتظر تأكيد أقوى |
| ❌ **تجنب** | تصريف عند دعم، أو مقاومة + تصريف | لا تشترِ — احتمال فخ |
| ⚠️ **تأكد** | كسر/اختراق منطقة، أو لا توجد بيانات Order Flow | شغّل Order Flow أو انتظر التأكيد |

### 💭 السبب (شرح مختصر)
- **دعم + تجميع + مشترين 🟢🟢** — أقوى إشارة ادخل
- **دعم + تجميع** — قوي لكن بدون تأكيد المشترين
- **دعم + مشترين** — متوسط، يحتاج تأكيد المرحلة
- **تصريف عند دعم — فخ** — السعر عند دعم لكن المؤسسات تبيع → لا تنخدع
- **مقاومة + تصريف + بائعين** — أقوى إشارة تجنّب
- **احتمال اختراق صاعد** — مقاومة مع تجميع = قد ينكسر للأعلى
- **كسر/اختراق المنطقة** — حركة قوية، تأكد إن كانت حقيقية
- **يقترب من المنطقة** — الفرصة لم تأتِ بعد
- **بدون Order Flow** — اضغط الزر العلوي لتفعيل التحليل الكامل
""")
                ...
                for r in _purple_rows_full:
                    ...
                    if _is_support and _zone_st in ('✅', '🔄'):
                        if _phase_raw in ('accumulation', 'spring') and _aggr_raw == 'buyers':
                            ...
                            _action = "❌ تجنب"
                            _reason = "تصريف عند دعم — فخ"
                        else:
                            _action = "⚠️ تأكد"
                            _reason = "دعم بدون Order Flow"
                    elif (not _is_support) and _zone_st in ('✅', '🔄'):
                        if _phase_raw in ('distribution', 'upthrust') and _aggr_raw == 'sellers':
                            _action = "❌ تجنب"
                            _reason = "مقاومة + تصريف + بائعين"
                        elif _phase_raw in ('distribution', 'upthrust'):
                            _action = "❌ تجنب"
                            _reason = "مقاومة + تصريف"
                        elif _phase_raw in ('accumulation', 'spring'):
                            _action = "⚠️ راقب"
                            _reason = "احتمال اختراق صاعد"
                        else:
                            _action = "⚠️ راقب"
                            _reason = "مقاومة"
                    elif _zone_st == '💥':
                        _action = "⚠️ تأكد"
                        _reason = "كسر/اختراق المنطقة"
                    elif _zone_st == '🎯':
                        _action = "⚠️ راقب"
                        _reason = "يقترب من المنطقة"
