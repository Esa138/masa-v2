...
if 'wa_sent' not in st.session_state:
    st.session_state.wa_sent = set()

with st.expander("⚙️ لوحة التحكم والإعدادات (المحفظة والواتساب)", expanded=False):
    ...
    with c_set2:
        st.markdown(
            "<h4 style='color:#25D366; text-align:right;'>📱 إشعارات الواتساب</h4>",
            unsafe_allow_html=True
        )
        wa_phone = st.text_input("📞 رقم الجوال (مع رمز الدولة)", placeholder="966500000000")
        wa_apikey = st.text_input("🔑 CallMeBot API Key", type="password")
        st.markdown(
            "<div style='font-size:11px; color:#888; direction:rtl; margin-top:4px;'>"
            "للحصول على المفتاح: أرسل <b>I allow callmebot to send me messages</b> "
            "عبر واتساب للرقم <b>+34 644 71 84 74</b></div>",
            unsafe_allow_html=True,
        )
    st.markdown(
        ...
        if not df_ai_picks.empty:
            ...
            if not df_vip.empty:
                ...
                for _, row in df_vip.iterrows():
                    ...
                    if wa_phone and wa_apikey and alert_id not in st.session_state.wa_sent:
                        # Build news line for WhatsApp
                        wa_news_line = ""
                        wa_news_sent = row.get('news_sentiment', 'محايد')
                        wa_news_sum = str(row.get('news_summary', ''))
                        if wa_news_sent != 'محايد' and wa_news_sum and wa_news_sum != 'لا توجد أخبار':
                            wa_news_line = f"📰 خبر: {wa_news_sum}\n"

                        # Build wolf line for WhatsApp
                        wa_wolf_line = ""
                        if row.get('is_wolf', False):
                            w_type = row.get('wolf_type', '')
                            w_rr = row.get('wolf_rr', 0)
                            wa_wolf_line = f"🐺 Wolf {'مؤكد' if w_type == 'مؤكد' else 'Breakout'} (R:R 1:{w_rr:.1f})\n"

                        # Build keyword alert line for WhatsApp
                        wa_kw_line = ""
                        ...
                        if 'خطر' in kw_v:
                            ...
                            wa_kw_line = f"💣 كلمات: {top_kw}\n"
                        elif 'إيجابي' in kw_v:
                            kw_hits_list = row.get('rocket_hits', [])
                            top_kw = ", ".join(str(h[0]) for h in kw_hits_list[:3]) if kw_hits_list else ""
                            wa_kw_line = f"🚀 كلمات: {top_kw}\n"

                        # Confluence line for WhatsApp
                        wa_conf_line = ""
                        wa_stars = int(row.get('confluence_stars', 0))
                        if wa_stars >= 3:
                            wa_signals = row.get('confluence_signals', [])
                            wa_sig_str = " + ".join(str(s) for s in wa_signals) if wa_signals else ""
                            wa_conf_line = f"{'⭐' * wa_stars} تقاطع: {wa_sig_str}\n"

                        # Sector line for WhatsApp
                        wa_sector_line = ""
                        wa_sector = str(row.get('stock_sector', ''))
                        if wa_sector:
                            wa_sector_line = f"🏷️ قطاع: {wa_sector}\n"

                        msg = (
                            f"🚨 Masa VIP Alert! 💎\n\n"
                            f"📌 {row['الشركة']} ({row['الرمز']})\n"
                            f"{wa_sector_line}"
                            f"⏱️ فريم: {tf_choice}\n"
                            f"💰 السعر: {row['السعر']}\n"
                            f"🎯 الهدف: {row['الهدف 🎯']}\n"
                            f"🛡️ الوقف: {row['الوقف 🛡️']}\n"
                        ...
                        try:
                            ...
                            requests.get(
                                f"https://api.callmebot.com/whatsapp.php?phone={wa_phone}&text={encoded_msg}&apikey={wa_apikey}",
                                timeout=15,
                            )
                            st.session_state.wa_sent.add(alert_id)
