...
### المؤشر المركب وحالة السوق (`market_breadth`):
في البيانات تجد `market_breadth` وفيه:
- `avg_flow_bias`: متوسط Order Flow لكل السوق (من -100 إلى +100)
- `accumulation_pct`: نسبة الأسهم في تجميع
- `distribution_pct`: نسبة الأسهم في تصريف
- `markup_pct`: نسبة الأسهم في صعود
- `verdict`: حكم نصي على السوق

استخدم هذي البيانات لما المستخدم يسأل عن:
- "حالة السوق العامة"
- "المؤشر المركب"
- "هل السوق صاعد ولا هابط؟"
- "نسبة التجميع في السوق"
- "PFI" أو "اتساع السوق" (market breadth)

### الأسهم الكاملة (`all_stocks`):
حقل `all_stocks` فيه قائمة بكل الأسهم في المسح (decision, phase, flow, rsi, change_pct, price, zr_status).
استخدمها لما المستخدم يسأل عن سهم محدد بالاسم أو الرمز — حتى لو ما كان في قائمة `enter_signals`.

### الأكثر صعوداً/هبوطاً (`top_gainers` / `top_losers`):
- `top_gainers`: أعلى 5 أسهم صعوداً اليوم
- `top_losers`: أعلى 5 أسهم هبوطاً اليوم
...
def _is_golden_signal(r: dict) -> bool:
    """Golden Filter: accumulation/spring + buyer + div>=25 + 0 against + RSI>50 + price>SMA600."""
    ...
    # Price above SMA 600 — strong long-term uptrend
...
def _summarize_scan(scan_results: list, max_items: int = 30) -> dict:
    ...
    avg_flow = sum(r.get("flow_bias", 0) for r in scan_results) / len(scan_results)
    accum_count = sum(1 for r in scan_results if r.get("phase") in ("accumulation", "spring"))
    dist_count = sum(1 for r in scan_results if r.get("phase") in ("distribution", "markdown"))
    markup_count = sum(1 for r in scan_results if r.get("phase") == "markup")

    # Top movers
    sorted_by_change = sorted(scan_results, key=lambda r: r.get("change_pct", 0), reverse=True)
    top_gainers = [
        {"name": r.get("name", ""), "ticker": r.get("ticker", ""), "change": r.get("change_pct", 0), "flow": r.get("flow_bias", 0)}
        for r in sorted_by_change[:5] if r.get("change_pct", 0) > 0
    ]
    top_losers = [
        {"name": r.get("name", ""), "ticker": r.get("ticker", ""), "change": r.get("change_pct", 0), "flow": r.get("flow_bias", 0)}
        for r in sorted_by_change[-5:] if r.get("change_pct", 0) < 0
    ...
    # Compact ALL stocks index — minimal data for lookup queries
    all_stocks_index = []
    for r in scan_results:
        all_stocks_index.append({
            "ticker": r.get("ticker", ""),
            "name": r.get("name", ""),
            "sector": r.get("sector", ""),
    ...
    return {
        ...
        "market_breadth": {
            "avg_flow_bias": round(avg_flow, 1),
            "accumulation_pct": round(accum_count / len(scan_results) * 100, 1),
            "distribution_pct": round(dist_count / len(scan_results) * 100, 1),
            "markup_pct": round(markup_count / len(scan_results) * 100, 1),
            "verdict": (
                "إيجابي قوي" if avg_flow > 20 else
                "إيجابي" if avg_flow > 5 else
                "محايد" if avg_flow > -5 else
                "سلبي" if avg_flow > -20 else "سلبي قوي"
            ),
        },
        "decisions": decisions,
        "top_sectors": [
            {"sector": s, "avg_flow": round(d["flow_sum"] / d["n"], 1), "enters": d["enters"], "n": d["n"]}
            for s, d in sector_ranks[:7]
        ],
        "weak_sectors": [
        ...
        "all_stocks": all_stocks_index,  # Full index for lookup queries
    }
